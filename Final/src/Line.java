...
public class Line extends Tetris {
	...
	public boolean rotation(int count, int num) {
		...
		switch (count % 2) {
		case 1:
			if (Main.y[num][1] + 2 > Main.colSize - 1)
				return false;
			if (Main.y[num][1] - 1 < 0)
				return false;
			if (Main.player[num][Main.x[num][1]][Main.y[num][1] + 2] > 0)
				return false;
			if (Main.player[num][Main.x[num][1]][Main.y[num][1] + 1] > 0)
				return false;
			if (Main.player[num][Main.x[num][1]][Main.y[num][1] - 1] > 0)
				return false;
			/////////////////////////////////////////////////
			...
			break;
		case 0:
			if (Main.x[num][1] + 2 > Main.rowSize - 1)
				return false;
			if (Main.x[num][1] - 1 < 0)
				return false;
			if (Main.player[num][Main.x[num][1] + 2][Main.y[num][1]] > 0)
				return false;
			if (Main.player[num][Main.x[num][1] + 1][Main.y[num][1]] > 0)
				return false;
			if (Main.player[num][Main.x[num][1] - 1][Main.y[num][1]] > 0)
				return false;
			///////////////////////////////////////////////////
